...
echo "Population Simulation Log Start..." >& "${RESULT_FOLDER}/logfile"
...
if [[ "${NUMBER_OF_CPU}" -gt  "${MAX_CPU}" ]]; then
  echo "The processor requested exceed the number of maximum of CPU in this PC!" >> "${RESULT_FOLDER}/logfile" 2>&1
  exit 1
fi
