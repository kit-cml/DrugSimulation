                   ...
                   const Parameter *p_param, Cipa_Features &p_features, short sample_id)
{
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_vmdebug_smp%d_%s.csv",
           drug_name, conc, drug_name, conc, sample_id, user_name);
  fp_vmdebug = fopen(buffer, "w");
  fprintf(fp_vmdebug, "%s,%s,%s,%s,%s,%s,%s,%s\n",
          "Pace", "T_Peak", "Vmpeak", "Vmvalley", "Vm_repol30", "Vm_repol50", "Vm_repol90", "dVmdt_repol_max");
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_last_10_paces_smp%d_%s.csv",
           drug_name, conc, drug_name, conc, sample_id, user_name);
  ...
  while (tcurr < tmax)
  {
    ...
    {
      cvar_end_of_cycle_funct(&pace_count, p_cell, p_param, p_features, temp_features, fp_vmdebug);
    ...
    {
      cvar_get_dvmdt_repol_max(p_cell, temp_features, p_param, tcurr, pace_count);
    ...
    {
      if (tcurr > bcl * (pace_max - 10) || tcurr <= bcl)
      {
        if (tcurr <= bcl)
        {
          tprint = tcurr;
        ...
        snprintf(buffer, sizeof(buffer), "%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf\n",
                 ...
                 p_cell->ALGEBRAIC[IK1] * cml::math::MICRO_TO_NANO);
  ...
  fprintf(fp_vmdebug, "%hd,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf\n",
          p_features.pace_target, p_features.time_vm_peak, p_features.vm_peak, p_features.vm_valley,
          p_features.vm_amp30, p_features.vm_amp50, p_features.vm_amp90,
...
void cvar_end_of_cycle_funct(short *pace_count, Cellmodel *p_cell, const Parameter *p_param, Cipa_Features &p_features,
                             Cipa_Features &temp_features, FILE *fp_vmdebug)
{
  if (*pace_count >= p_param->pace_max - 250)
  {

    fprintf(fp_vmdebug, "%hd,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf\n",
            *pace_count, temp_features.time_vm_peak, temp_features.vm_peak, temp_features.vm_valley,
            temp_features.vm_amp30, temp_features.vm_amp50, temp_features.vm_amp90,
