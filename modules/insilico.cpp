...
const Parameter *p_param, Cipa_Features &p_features, short sample_id)
{
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.0lf/%s_%.0lf_vmdebug_smp%d_%s.csv",
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.0lf/%s_%.0lf_last_10_paces_smp%d_%s.csv",
        drug_name,conc, drug_name, conc, sample_id,user_name );
...
void end_of_cycle_funct(short *pace_count, Cellmodel *p_cell, const Parameter *p_param, Cipa_Features &p_features, 
