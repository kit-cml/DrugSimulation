...
void insilico(double conc, const Drug_Row &hill, const Drug_Row &herg, const Parameter *p_param, Cipa_Features &p_features, short sample_id,
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_vmdebug_smp%d_%s.csv", drug_name, conc, drug_name, conc, sample_id, user_name);
  fp_vmdebug = fopen(buffer, "w");
  fprintf(fp_vmdebug, "%s,%s,%s,%s,%s,%s,%s,%s\n", "Pace", "T_Peak", "Vmpeak", "Vmvalley", "Vm_repol30", "Vm_repol50", "Vm_repol90",
          "dVmdt_repol_max");
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_last_states_smp%d_%s.csv", drug_name, conc, drug_name, conc, sample_id, user_name);
  fp_last_states = fopen(buffer, "w");

  FILE *fp_last_ten_paces;
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_last_10_paces_smp%d_%s.csv", drug_name, conc, drug_name, conc, sample_id, user_name);
