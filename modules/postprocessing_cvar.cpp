#include "postprocessing_cvar.hpp"
...
void postprocessing_cvar(double conc, double inal_auc_control, double ical_auc_control,
...
const Parameter *p_param, Cipa_Features &p_features, short sample_id, short group_id)
{
  ...
  p_cell->initConsts((double)celltype, conc, hill.data, herg.data, cvar.data);
#elif defined TOMEK_2019
  const char CELL_NAME[] = "tomek";
  printf("Using Tomek 2019 cell model\n");
  p_cell = new Tomek_model();
  p_cell->initConsts((double)celltype, conc, hill.data, cvar.data);
#else
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_last_states_smp%d_%s.csv",
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_time_series_smp%d_%s.csv",
        drug_name,conc, drug_name, conc, sample_id,user_name);
  ...
  while( tcurr < tmax ){
    ...
    get_vm_features_postprocessing_cvar( p_cell, p_features, tcurr );
    get_ca_features_postprocessing_cvar( p_cell, p_features, tcurr );
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_features_core%d_%s.csv",
        p_param->drug_name,conc, p_param->drug_name, conc, MPI_Profile::rank,p_param->user_name);
  collect_features_cvar( p_features, p_param, p_cell, conc, inet_auc, inet_apd_auc, inal_auc, ical_auc, inal_auc_control, ical_auc_control, buffer, sample_id, group_id);
...
short set_initial_condition_postprocessing_cvar(Cellmodel *p_cell, const char *ic_file_name)
{
  char buffer[50];
  FILE *fp_states;
  short idx;
...
void get_vm_features_postprocessing_cvar( Cellmodel *p_cell, Cipa_Features &p_features, const double tcurr )
{
...
void get_ca_features_postprocessing_cvar( Cellmodel *p_cell, Cipa_Features &p_features, const double tcurr )
{
...
void get_duration_postprocessing_cvar( Cipa_Features &p_features )
{
...
void collect_features_cvar( Cipa_Features &p_features, const Parameter *p_param, Cellmodel *p_cell, double conc, double inet_auc, double inet_apd_auc, double inal_auc, double ical_auc, double inal_auc_control, double ical_auc_control, char *features_file_name, short sample_id, short group_id)
{
  ...
  get_duration_postprocessing_cvar( p_features );
