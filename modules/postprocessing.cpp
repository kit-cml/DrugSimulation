...
void postprocessing(double conc, double inal_auc_control, double ical_auc_control, const Drug_Row &hill, const Drug_Row &herg,
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_time_series_smp%d_%s.csv", drug_name, conc, drug_name, conc, sample_id, user_name);
  ...
  snprintf(buffer, sizeof(buffer), "result/%s/%.2lf/%s_%.2lf_features_core%d_%s.csv", p_param->drug_name, conc, p_param->drug_name, conc,
