...
echo "[1/1] Running simulation with mpiexec in $SIM_DIR"
cd "/workspace/DrugSimulation/$SIM_DIR"

if [[ ! -x "../drugsim_${BIN_SUFFIX}" ]]; then
  echo "Missing binary ../drugsim_${BIN_SUFFIX}"
  echo "Build first with docker/linux/build_and_run.sh or make in the container."
  exit 3
fi
